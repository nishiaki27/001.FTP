...
class SC_Helper_PageLayout {
     ...
     * @return void
     */
    function sfGetPageLayout(&$objPage, $preview = false, $url = "", $device_type_id = DEVICE_TYPE_PC , $url2 = "") {
        ...
        if ($preview === false) {
            ...
            if(!$arrPageData){
    	       // print_r($url2);
    	        $url2 = preg_replace('|^' . preg_quote(ROOT_URLPATH) . '|', '', $url2);
	            $arrPageData = $this->getPageProperties($device_type_id, null, 'url = ?', array($url2));
        	    //print_r($arrPageData);
