...
class SC_Helper_Purchase {
    ...
    function getDateArray($start_day, $end_day) {
        ...
        if($start_day >= 1) {
            ...
            if (($y==2013)&&($m==2)) {
                if (($d==3)||($d==10)||($d==11)||($d==17)||($d==24)) {
                    $flg_stop=false;
                }
            }
            if (($y==2013)&&($m==3)) {
                if (($d==3)||($d==10)||($d==17)||($d==20)||($d==24)||($d==31)) {
                    $flg_stop=false;
                }
            }
            if (($y==2013)&&($m==4)) {
                if (($d==7)||($d==14)||($d==21)||($d==28)||($d==29)) {
                    $flg_stop=false;
                }
            }
            if (($y==2013)&&($m==5)) {
                if (($d==3)||($d==4)||($d==5)||($d==6)||($d==12)||($d==19)||($d==26)) {
                    $flg_stop=false;
                }
            }
            if (($y==2013)&&($m==6)) {
            ...
            if (($y==2013)&&($m==9)) {
                if (($d==1)||($d==8)||($d==15)||($d==16)||($d==22)||($d==23)||($d==29)) {
                    $flg_stop=false;
                }
            }
            if (($y==2013)&&($m==10)) {
                if (($d==6)||($d==13)||($d==14)||($d==20)||($d==27)) {
                    $flg_stop=false;
                }
            }
            if (($y==2013)&&($m==11)) {
                if (($d==3)||($d==4)||($d==10)||($d==17)||($d==23)||($d==24)) {
                    $flg_stop=false;
                }
            }
            if (($y==2013)&&($m==12)) {
                if (($d==1)||($d==8)||($d==15)||($d==22)||($d==23)||($d==29)||($d==30)||($d==31)) {
                    $flg_stop=false;
                }
            }
                if ($flg_stop) {
    ...
    function registerOrderComplete($orderParams, &$objCartSession, $cartKey) {
        ...
        foreach ($cartItems as $item) {
            ...
            $arrDetail[$i]['attribute'] = $item['attribute'];
            $arrDetail[$i]['attribute2'] = $item['attribute2'];
...
__EOS__;
    ...
    function getKikanData($order_id, $has_items = true) {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $arrResults = array();
        $arrProCodes = $objQuery->select("product_code", "dtb_order_detail", "order_id = ?", $order_id);
        // product_code ごとの配列を生成する
        foreach ($arrProCodes as $proCd) {
            //各product_codeから基幹データを取得;
            //$arrKikanDatas = $objQuery->select("*", "dtb_kikan_data", "product_code = ? AND del_flg = ? AND kikan_id is not null", array($proCd['product_code'], 0));

            // クエリ文
            $sql = "SELECT *, product_id_3 AS product_id, product_code_3 AS product_code FROM kikan_data WHERE product_code_3 = :product_code AND del_flg = 0 AND kikan_id is not null;";
            $param = array();
            $param[':product_code'] = $proCd['product_code'];
            $arrKikanDatas = $this->getKikanShareData($sql, $param);

            $cnt = 0;
            if ($arrKikanDatas) {
                //あれば配列で各仕入先などを取得
                foreach ($arrKikanDatas as $kikan) {
                    $arrKikanData[$kikan["product_code"]]["kikan_id"][$cnt] = $kikan["kikan_id"];
                    $arrKikanData[$kikan["product_code"]]["maker_name"][$cnt] = $kikan["maker_name"];
                    $arrKikanData[$kikan["product_code"]]["supplier_name"][$cnt] = $kikan["supplier_name"];
                    $arrKikanData[$kikan["product_code"]]["kikan_code"][$cnt] = $kikan["kikan_code"];
                    if ($kikan["supplier_name"] == "三冷社") {
                        $code = $proCd['product_code'];
                        if (strpos($code,'white') !== false) {
                            $arrKikanData[$kikan["product_code"]]["meisai"] = "ﾎﾜｲﾄﾊﾟﾈﾙ ";
                        }
                        if (strpos($code,'airflex') !== false) {
                            $arrKikanData[$kikan["product_code"]]["meisai"] = "ｴｱﾌﾚｯｸｽﾊﾟﾈﾙ ";
                        }
                        if (strpos($code,'raku') !== false) {
                            $arrKikanData[$kikan["product_code"]]["meisai"] = "ﾗｸﾘｰﾅﾊﾟﾈﾙ ";
                        }
                        if (strpos($code,'osouji') !== false) {
                            $arrKikanData[$kikan["product_code"]]["meisai"] = "お掃除ﾗｸﾘｰﾅ ";
                        }
                        if (strpos($code,'canvas') !== false) {
                            $arrKikanData[$kikan["product_code"]]["meisai"] = "ｷｬﾝﾊﾞｽﾀﾞｸﾄ";
                        }
                        if (strpos($code,'silent') !== false) {
                            $arrKikanData[$kikan["product_code"]]["meisai"] = "ｻｲﾚﾝﾄﾊﾟﾈﾙ ";
                        }
                    }
                    $cnt++;
                }
                ...
                //なければ基幹IDを空にする
                $arrKikanData[$proCd['product_code']]["kikan_id"][$cnt] = "";
                $arrKikanData[$proCd['product_code']]["supplier_name"] = array("東芝キャリア","山善","ユアサ","三冷社","冷販","九州日立","福西","冨士機材");
                $arrKikanData[$proCd['product_code']]["supplier_id"] = array("10122","8005","10568","10521","10417","6010","6033","11243");
            }
        }
        return $arrKikanData;
    }

    function getKikanShareData($sql, $param) {
        try {
            $dbh = new PDO('mysql:host=' . AC_DB_SERVER . ';dbname=' . AC_DB_NAME . ';charset=utf8', AC_DB_USER, AC_DB_PASSWORD, array(PDO::ATTR_PERSISTENT => true, PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
