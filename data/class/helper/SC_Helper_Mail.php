...
#        $objPage->tpl_header = $arrRet[0]['header'];
        $objPage->tpl_header = preg_replace('/◎ご入金の際、弊社からご案内しているご注文番号を/', '◎ご入金の際、弊社からご案内しているご注文番号の下5桁【'.substr($order_id, -5).'】を', $arrRet[0]['header']);
    ...
    /* 受注完了メール送信 */
    function sfSendOrderMail($order_id, $template_id, $subject="", $header="", $footer="", $send=true, $attachment=array()) {
...
#            $arrTplVar->tpl_header = $arrRet[0]['header'];
            $arrTplVar->tpl_header = preg_replace('/◎ご入金の際、弊社からご案内しているご注文番号を/', '◎ご入金の際、弊社からご案内しているご注文番号の下5桁【'.substr($order_id, -5).'】を', $arrRet[0]['header']);
            $arrTplVar->tpl_footer = $arrRet[0]['footer'];
            $tmp_subject = $arrRet[0]['subject'];
        } else {
#            $arrTplVar->tpl_header = $header;
            $arrTplVar->tpl_header = preg_replace('/◎ご入金の際、弊社からご案内しているご注文番号を/', '◎ご入金の際、弊社からご案内しているご注文番号の下5桁【'.substr($order_id, -5).'】を', $header);
        ...
        //DM発送設定
        if (preg_match('/.*【DM発送】受け取る.*/',$arrOrder['note'])) {
            $arrOrder['dm'] = "受け取る";
        }
        if (preg_match('/.*【DM発送】受け取らない.*/',$arrOrder['note'])) {
            $arrOrder['dm'] = "受け取らない";
        }
        ...
        $objSendMail->setItem('', $tosubject, $body, $from, $arrInfo['shop_name'], $from, $error, $error, $bcc, '', $attachment);
        $objSendMail->setTo($arrOrder["order_email"], $arrOrder["order_name01"] . " ". $arrOrder["order_name02"] ." 様");

        // 送信フラグ:trueの場合は、送信する。
        if($send) {
            $tosubject = $tosubject . "No." . $order_id;
            $objSendMail->setItem('', $tosubject, $body, $from, $arrInfo['shop_name'], $from, $error, $error, $bcc, '', $attachment);
            ...
            if ($objSendMail->sendMail(false, $isAttached)) {
                $this->sfSaveMailHistory($order_id, $template_id, $tosubject, $body, $attachment);
            }
        }
    ...
    // メール配信履歴への登録
    function sfSaveMailHistory($order_id, $template_id, $subject, $body, $attachment=array()) {
        ...
        foreach ($attachment as $key => $value) {
            $sqlval2 = array();
            $sqlval2['temp_id'] = $objQuery->nextVal("dtb_mail_history_temp_pkey");
            $sqlval2['send_id'] = $sqlval['send_id'];
            $sqlval2['save_path'] = $value['file_path_' . $key];
            $sqlval2['save_file'] = $value['file_name_tmp_' . $key];
            $sqlval2['upload_file'] = $value['file_name_' . $key];
            $objQuery->insert("dtb_mail_history_temp", $sqlval2);
        }
    }
