 ...
 */
class LC_Page_Building extends LC_Page_Ex {
    ...
    function init() {
        ...
        if (SC_Display_Ex::detectDevice() == DEVICE_TYPE_MOBILE) {
            switch ($_GET['maker_name']) {case 'daikin':$this->tpl_title = 'ダイキン　ビル用マルチエアコン';
  	    break;case 'toshiba':$this->tpl_title = '東芝　ビル用マルチエアコン';
  	    break;case 'mitsubishidenki':$this->tpl_title = '三菱電機　ビル用マルチエアコン';
  	    break;case 'hitachi':$this->tpl_title = '日立　ビル用マルチエアコン';
  	    break;case 'mitsubishijyuko':$this->tpl_title = '三菱重工　ビル用マルチエアコン';
  	    break;default:$this->tpl_title = 'お見積り申込みフォーム';
};
        } else {
/*
        switch ($_GET['maker_name']) {case 'daikin':$this->tpl_title = 'ダイキン　ビル用マルチエアコン';
...
*/
				$this->tpl_title = 'お見積り申込みフォーム';
        }
        $this->tpl_page_category = 'building';
    ...
    function action() {
        ...
        $this->arrData = isset($_SESSION['building']) ? $_SESSION['building'] : "";
        ...
        case 'confirm':
            ...
            if($this->arrErr["contact"]==""){
	            if($_POST['contact'] == "電話連絡希望" ||$_POST['contact'] == "どちらでもよい"){
	            	$tel_err=0;
	            	if($_POST["tel01"]==""){ $this->arrErr["tel01"]="tel01";	$tel_err++;}
	            	if($_POST["tel02"]==""){ $this->arrErr["tel02"]="tel02";	$tel_err++;}
	            	if($_POST["tel03"]==""){ $this->arrErr["tel03"]="tel03";	$tel_err++;}
	            	if($tel_err>0){
	            		$this->arrErr["tel"]="※電話番号が入力されていません。<br>";
	            	}
	            }
	            if($_POST['contact'] == "メール連絡希望" ||$_POST['contact'] == "どちらでもよい"){
	            	if($_POST["email"]==""){
	            		$this->arrErr["email"]="※メールアドレスが入力されていません。<br>";
	            	}
	            }
            }
            ...
            if (SC_Utils_Ex::isBlank($this->arrErr)) {
                // エラー無しで完了画面
                $this->tpl_mainpage = 'building/confirm.tpl';
                $this->tpl_title = 'お問い合わせ(確認ページ)';
								$this->tpl_title = '在庫/納期確認クイックフォーム(完了ページ)';
            }
    ...
    function lfInitParam(&$objFormParam) {
        ...
        $objFormParam->addParam("住所", "addr01", MTEXT_LEN, 'KVa', array("EXIST_CHECK","SPTAB_CHECK" ,"MAX_LENGTH_CHECK"));
        ...
        $objFormParam->addParam('希望連絡方法', 'contact', null, 'KVa',array("EXIST_CHECK"));
        
        $objFormParam->addParam('メールアドレス', 'email', null, 'KVa',array("EMAIL_CHECK", "EMAIL_CHAR_CHECK"));
...
$sqlval['contents']    = "【希望連絡方法】 ".$this->arrForm['contact']['value']."\n".$this->arrForm['contents']['value'];    //問い合わせ内容
