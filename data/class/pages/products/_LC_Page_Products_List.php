...
//リスト絞込み処理ここから（20120110）
if(preg_match("/op/i", $_SERVER['QUERY_STRING'])){
	preg_match("/category_id=[0-9]+/", $_SERVER['QUERY_STRING'], $type);
	$type = str_replace('category_id=',"",$type[0]);

	preg_match("/op_4=[0-9]+/", $_SERVER['QUERY_STRING'], $op4);
	$op4 = str_replace('op_4=',"",$op4[0]);

	preg_match("/op_5=[0-9]+/", $_SERVER['QUERY_STRING'], $op5);
	$op5 = str_replace('op_5=',"",$op5[0]);

	preg_match("/op_6=[0-9]+/", $_SERVER['QUERY_STRING'], $op6);
	$op6 = str_replace('op_6=',"",$op6[0]);

	preg_match("/mode=[a-z]+/", $_SERVER['QUERY_STRING'], $mode);
	$mode = str_replace('mode=',"",$mode[0]);

	preg_match("/name=(.*?)&/i", $_SERVER['QUERY_STRING'], $name);
	preg_match("/name=(.*?)$/i", $_SERVER['QUERY_STRING'], $name);
	$name = urldecode(str_replace('&', "", str_replace('name=',"",$name[0])));
	...
	$op_array = array($op4_query,$op5_query,$op6_query);
	$op_str = implode(" ", $op_array);

	$this->arrForm['name'] = $op_str;
}
//処理ここまで
      ...
      $subtitle = explode(" ",$this->tpl_subtitle);
			$this->tpl_subtitle3 =$subtitle[0];
	    //タイトル用リストを取得
    	$this->arrTitlePath = $this->lfTitlePath($this->mode, $this->arrForm['category_id'], $this->arrForm['name']);
...
__EOS__;
        ...
        // 旧型番検索 2017/08/05
        if ($this->arrForm['search_x'] != '' && $this->arrForm['search_y'] != '' && $this->arrForm['name'] != '') {
            $searchCondition['where'] = $this->setOldTypeSearchQuery($searchCondition, $objQuery, $objProduct, $arrval_order);
        }
     ...
     * @return void
     */
    function setOldTypeSearchQuery($searchCondition, $objQuery, $objProduct, $arrval_order) {
        // 他処理へ影響のないように複製
        $searchCondition_cp = $searchCondition;
        $searchCondition_cp['where'] = str_replace("comment1 IS NULL", "comment1 = '非表示'", $searchCondition_cp['where']);

        $objQuery->setWhere($searchCondition_cp['where']);
        $objQuery->setLimit(10);

        // 表示すべきIDとそのIDの並び順を一気に取得
        $arrProductId = $objProduct->findProductIdsOrder($objQuery, array_merge($searchCondition_cp['arrval'], $arrval_order));
        ...
        if (!empty($arrProducts)) {
            foreach ($arrProducts as $arrProduct) {
                if ($arrProduct['product_code_max'] == $this->arrForm['name']) {
                    $searchCondition['where'] = $searchCondition_cp['where'];
                    $this->tpl_linemax = $this->lfGetProductAllNum($searchCondition);
                    break;
                }
            }
        }
        return $searchCondition['where'];
    }
    ...
    function lfGetSearchCondition($arrSearchData){
        ...
        if($_GET["above_price"]){
        	$above_price=$_GET["above_price"]*1.08;
	        $searchCondition['where'] .= " AND T1.price02 >= ".$above_price;
				}
...
//        }
        // 20120303 - 在庫無し商品を検索結果から非表示にする為に、comment1に文字列が入っていると表示されないように処理
        if (NOSTOCK_HIDDEN) {
            $searchCondition['where'] .= ' AND alldtl.comment1 IS NULL';
        }
	...
	$name_query = urldecode($_GET['name']);

        $name = $arrSearchData['name'] . " " . $name_query;
...
//馬力部分の処理の為作業モードを2重化　20120125
if ($_GET['mode'] == "power") {
        // 分割したキーワードを一つずつwhere文に追加
        foreach ($names as $val) {
            if ( strlen($val) > 0 ) {
                //$searchCondition['where']    .= " AND ( alldtl.name ILIKE ? OR alldtl.comment3 ILIKE ?) ";
		if(preg_match("/^not/", $val)){

			$notval = ereg_replace("not", "", $val);
			//キーワードの頭がnotだった場合、そのキーワードを除外扱いする
        		$searchCondition['where'] .= " AND  alldtl.comment3 NOT LIKE '%$notval%' ";

		}else{

	        // 各種コメントを追加
        		$searchCondition['where'] .= " AND alldtl.comment3 ILIKE '%$val%' ";
		}
        ...
        foreach ($names as $val) {
            if ( strlen($val) > 0 ) {
                //$searchCondition['where']    .= " AND ( alldtl.name ILIKE ? OR alldtl.comment3 ILIKE ?) ";
	        // 各種コメントを追加
        	$searchCondition['where'] .= " AND ( alldtl.name ILIKE ? OR alldtl.comment3 ILIKE ? OR alldtl.comment4 ILIKE ?) ";
            ...
            if($val != $category_id){
            	if( $val == 100000){
	              $arrRet[] = '';
		              ...
		              $arrRet[] = '<a href="http://test.tokyo-aircon.net/products/list.php?category_id=' .$val. '&orderby=price">'. $arrCatName['name'] . '</a> > ';
		            }
	            }
            } else {
     ...
     * @return array
     */
    function lfTitlePath($mode, $category_id, $category_name){
        if ($mode == 'search') {
          $arrRet = $category_name . " の検索結果";
        } elseif (empty($category_id)){
          $arrRet = "検索結果";
