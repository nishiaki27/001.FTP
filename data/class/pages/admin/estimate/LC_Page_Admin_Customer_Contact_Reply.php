...
class LC_Page_Admin_Customer_Contact_Reply extends LC_Page {
    function init() {
        parent::init();
        $this->tpl_mainpage = 'estimate/contact_reply.tpl';
        $this->tpl_mainno = 'estimate';
        $this->tpl_subnavi = 'estimate/subnavi.tpl';
        $this->tpl_subno = 'estimate';
        $this->tpl_subtitle = '見積り依頼へ返信';
        // アップロードファイル保存先
        $this->upload_filepath = '/data/upload/estimate_reply_temp';
...
//        $objQuery = new SC_Query_Ex();
        ...
        // お問合せ内容の取得
        $this->contact_data = $objQuery->select("*", "dtb_estimate", "contact_id=?", array($this->contact_id));
...
//                    $objSendMail->setItem($to, $subject, $body, $fromaddress, $from_name, $reply_to, $return_path, $errors_to, $bcc);
                    $objSendMail->setItem($to, $subject, $body, $reply_to, $from_name, $reply_to, $errors_to, $bcc, '', $attachment);
                    ...
                    // メール保存
                    $sqlval['mail_id']     = $objQuery->nextVal("dtb_estimate_reply_mail_id");
                    ...
                    $objQuery->begin();
                    $objQuery->insert("dtb_estimate_reply", $sqlval);
                    $objQuery->commit();

                    // 添付ファイル保存
                    foreach ($attachment as $key => $value) {
                        if ($value['file_name_' . $key]) {
                            $sqlval2['temp_id'] = $objQuery->nextVal("dtb_estimate_reply_temp_temp_id");
                            ...
                            $objQuery->begin();
                            $objQuery->insert("dtb_estimate_reply_temp", $sqlval2);
                            $objQuery->commit();
                        }
                    }
                    $this->uploaded = array();
                ...
                if ($_POST['select_file']) {
                    ...
                    $where = "temp_id = ?";
                    $arrMailTempData = $objQuery->select($col, "dtb_estimate_reply_temp", $where, array($_POST['select_file']));
...
//        $objQuery->unsetorder();
        ...
        if (isset($_GET['contact_id']) || $_POST['mode'] == "template" || $_POST['mode'] == "send") {
...
' . $this->mail_template[0]['footer'];
        ...
        $arrReplyData = $objQuery->select("*", "dtb_estimate_reply", "contact_id = ?", array($this->contact_id));
        ...
        foreach ($arrReplyData as $key => $value) {
            ...
            $arrReplyData[$key]['upload_file'] = $objQuery->select($col, "dtb_estimate_reply_temp", $where, array($value['mail_id']));
        }
