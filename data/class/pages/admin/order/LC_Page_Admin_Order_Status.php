 ...
 * @version $Id: LC_Page_Admin_Order_Status.php 21185 2011-08-11 10:37:10Z shutta $
 */
class LC_Page_Admin_Order_Status extends LC_Page_Admin_Ex {
    ...
    function action() {
        ...
        $this->lfStatusDisp($status, $objFormParam->getValue('search_pageno'));
 				...
 				//DM配送の設定
        $num =count($this->arrStatus);
        for($i=0;$i<$num;$i++){
        	if(preg_match('/.*【DM発送】受け取る.*/',$this->arrStatus[$i]['note'])){
        		$this->arrStatus[$i]['dm']="受け取る";
        	}
        	if(preg_match('/.*【DM発送】受け取らない.*/',$this->arrStatus[$i]['note'])){
        		$this->arrStatus[$i]['dm']="受け取らない";
        	}
        }
