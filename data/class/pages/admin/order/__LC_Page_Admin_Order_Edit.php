 ...
 * @version $Id: LC_Page_Admin_Order_Edit.php 21408 2012-01-17 07:24:22Z Seasoft $
 */
class LC_Page_Admin_Order_Edit extends LC_Page_Admin_Order_Ex {
    ...
    function action() {
		...
		// dtb_order_detailの読み込み
        $this->arrOrderDetail = $objPurchase->getOrderDetail($order_id, false);
    ...
    function lfInitParam(&$objFormParam) {
        ...
        // お客様情報
        $objFormParam->addParam("注文者 会社名", "order_name01", STEXT_LEN, 'KVa', array("SPTAB_CHECK", "MAX_LENGTH_CHECK"));
        $objFormParam->addParam("注文者 お名前", "order_name02", STEXT_LEN, 'KVa', array("EXIST_CHECK", "SPTAB_CHECK", "MAX_LENGTH_CHECK"));
        $objFormParam->addParam("注文者 会社名(フリガナ)", "order_kana01", STEXT_LEN, 'KVCa', array("SPTAB_CHECK", "MAX_LENGTH_CHECK"));
        $objFormParam->addParam("注文者 お名前(フリガナ)", "order_kana02", STEXT_LEN, 'KVCa', array("EXIST_CHECK", "SPTAB_CHECK", "MAX_LENGTH_CHECK"));
        ...
        $objFormParam->addParam("パネル", "attribute");	//2014-02-06追加
        $objFormParam->addParam("電源", "attribute2");	//2014-02-06追加
        ...
        $objFormParam->addParam("会社名", "shipping_name01", STEXT_LEN, 'KVa', array("SPTAB_CHECK", "MAX_LENGTH_CHECK"));
        $objFormParam->addParam("お名前", "shipping_name02", STEXT_LEN, 'KVa', array("SPTAB_CHECK", "MAX_LENGTH_CHECK"));
        $objFormParam->addParam("会社名(フリガナ)", "shipping_kana01", STEXT_LEN, 'KVCa', array("SPTAB_CHECK", "MAX_LENGTH_CHECK"));
        $objFormParam->addParam("お名前(フリガナ)", "shipping_kana02", STEXT_LEN, 'KVCa', array("SPTAB_CHECK", "MAX_LENGTH_CHECK"));
        ...
        $objFormParam->addParam("追加パネル", "add_attribute", STEXT_LEN, 'n', array("MAX_LENGTH_CHECK", "NUM_CHECK"));	//2014-02-24追加
        $objFormParam->addParam("追加電源", "add_attribute2", STEXT_LEN, 'n', array("MAX_LENGTH_CHECK", "NUM_CHECK"));	//2014-02-24追加

        $objFormParam->addParam("汎用項目", "memo01");	//2014-02-27追加
    ...
    function doRegister($order_id, &$objPurchase, &$objFormParam, &$message, &$arrValuesBefore) {
        ...
        $arrDetail = $objFormParam->getSwapArray(array("product_id",
                                                       ...
                                                       "classcategory_name1",
                                                       "classcategory_name2",//));
                                                       "attribute","attribute2"));	//2014-02-06追加
        ...
        for($i = 0; $i < $max; $i++) {
            if (!empty($arrDetail[$i]['product_id'])) {
                ...
                if (!empty($arrPreDetail) && $arrPreDetail[0]['quantity'] != $arrDetail[$i]['quantity']) {
					...
					// 2014/02/25　追加
                    $arrStockData[$k]['attribute'] = $objFormParam->getValue('add_attribute');
                    $arrStockData[$k]['attribute2'] = $objFormParam->getValue('add_attribute2');
    ...
    function doRegisterProduct(&$objFormParam) {
		...
		// 2014/02/25　追加
        $add_attribute = $objFormParam->getValue('add_attribute');
        $add_attribute2 = $objFormParam->getValue('add_attribute2');
        ...
        // 2014/02/25　追加
	    $attribute = $objFormParam->getValue('attribute');
        $attribute2 = $objFormParam->getValue('attribute2');

        foreach (array_keys($arrExistsProductClassIds) as $key) {
            $exists_product_class_id = $arrExistsProductClassIds[$key];
			
			$exists_attribute=$attribute[$key];
			$exists_attribute2=$attribute2[$key];
            if ($exists_product_class_id == $product_class_id && $exists_attribute == $add_attribute && $exists_attribute2 == $add_attribute2) {
                $exists = true;
                $exists_no = $key;
        ...
        if (!$exists) {
			...
			// 2014/02/25　追加
	        $arrProduct['attribute'] = $objFormParam->getValue('add_attribute');
	        $arrProduct['attribute2'] = $objFormParam->getValue('add_attribute2');
