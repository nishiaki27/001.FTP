 ...
 * @version $Id: LC_Page_Admin_Order_MailView.php 21250 2011-09-22 07:13:57Z nanasess $
 */
class LC_Page_Admin_Order_MailView extends LC_Page_Admin_Ex {
    ...
    function action() {
        ...
        if(SC_Utils_Ex::sfIsInt($send_id)) {
            ...
            $this->uploaded = $mailHistory[0]['upload_file'];
        }
    ...
    function getMailHistory($send_id){
        ...
        $col = "send_id, subject, mail_body";
        $where = "send_id = ?";
        $mailHistory = $objQuery->select($col, "dtb_mail_history", $where, array($send_id));

        // 添付ファイル取得
        foreach ($mailHistory as $key => $value) {
            $mailHistory[$key]['upload_file'] = array();

            $col = "temp_id, upload_file";
            $where = "send_id = ?";
            $objQuery->setOrder("temp_id ASC");
            $mailHistory[$key]['upload_file'] = $objQuery->select($col, "dtb_mail_history_temp", $where, array($value['send_id']));
        }
        return $mailHistory;
    }
