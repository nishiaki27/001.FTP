 ...
 * @version $Id: LC_Page_Admin_Order.php 21185 2011-08-11 10:37:10Z shutta $
 */
            ...
            case 'search':
                ...
                if (count($this->arrErr) == 0) {
                    ...
                    default:
                        ...
                        // DM配送の設定
                        $num = count($this->arrResults);
                        for ($i = 0; $i < $num; $i++) {
                            if (preg_match('/.*【DM発送】受け取る.*/', $this->arrResults[$i]['note'])) {
                                $this->arrResults[$i]['dm'] = "受け取る";
                            }
                            if (preg_match('/.*【DM発送】受け取らない.*/', $this->arrResults[$i]['note'])) {
                                $this->arrResults[$i]['dm'] = "受け取らない";
                            }
                        }
                    }
                }
...
/*
        ...
        $strOrderID= $objQuery->select($select, $from,"","");

        foreach($strOrderID as $key => $id){
            $select = "*";
            $where = "id = ?";
            $arrOrderMail= $objQuery->select($select, $from, $where, $strOrderID[$key]['id']);
            $arrZaikoMail[$strOrderID[$key]['id']] = $arrOrderMail[0];
            $arrZaikoMail[$strOrderID[$key]['id']]['line'] = substr_count($arrOrderMail[0]['text'], "\r\n") * 18; //改行数を取得（表示のpxように計算）
        }
*/
//-----------------------------
        //在庫状況の取得
        ...
        //表示順序
        $objQuery->setOrder($order);
        $arrOrderMail= $objQuery->select($select, $from, "", "");

        foreach ($arrOrderMail as $id => $key) {
            $arrZaikoMail[$key['id']] = $arrOrderMail[$id];
            $arrZaikoMail[$key['id']]['line'] = substr_count($key['text'], "\r\n") * 18; //改行数を取得（表示のpxように計算）
        }
//-----------------------------
        $this->arrZaiko=$arrZaikoMail;
