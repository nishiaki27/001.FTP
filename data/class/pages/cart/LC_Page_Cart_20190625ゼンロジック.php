 ...
 * @version $Id: LC_Page_Cart.php 21408 2012-01-17 07:24:22Z Seasoft $
 */
class LC_Page_Cart extends LC_Page_Ex {
    ...
    function action() {
        ...
        $attribute2 = $objFormParam->getValue('attribute2');
				$gencho_sign = $objFormParam->getValue('gencho_sign');
        ...
        case 'delete'://カートから削除
						...
						//
						$sessid_del = isset($_COOKIE["PHPSESSID"]) ? $_COOKIE["PHPSESSID"] : NULL;
						$product_id_del = $_SESSION['cart'][1][$cart_no]['productsClass']['product_id'];
						$attribute_del = $_SESSION['cart'][1][$cart_no]['attribute'];
						$attribute2_del = $_SESSION['cart'][1][$cart_no]['attribute2'];
						// PgSQLへ接続する
						...
						//dtb_genchoの商品データを空にする
						if( $attribute_del != "" && $attribute2_del == "" ){
							$sql_del = "DELETE FROM dtb_gencho WHERE sess_id ='".$sessid_del."' AND product_id =".$product_id_del." AND attribute ='".$attribute_del."'";
							$result_del = pg_query($link, $sql_del) or die("クエリの送信に失敗しました。<br />SQL:".$sql_del);
						}
						elseif( $attribute_del == "" && $attribute2_del != "" ){
							$sql_del = "DELETE FROM dtb_gencho WHERE sess_id ='".$sessid_del."' AND product_id =".$product_id_del." AND attribute2 ='".$attribute2_del."'";
							$result_del = pg_query($link, $sql_del) or die("クエリの送信に失敗しました。<br />SQL:".$sql_del);
						}
						elseif( $attribute_del != "" && $attribute2_del != "" ){
							$sql_del = "DELETE FROM dtb_gencho WHERE sess_id ='".$sessid_del."' AND product_id =".$product_id_del." AND attribute ='".$attribute_del."' AND attribute2 ='".$attribute2_del."'";
							$result_del = pg_query($link, $sql_del) or die("クエリの送信に失敗しました。<br />SQL:".$sql_del);
						}
						else{
							$sql_del = "DELETE FROM dtb_gencho WHERE sess_id ='".$sessid_del."' AND product_id =".$product_id_del."";
							$result_del = pg_query($link, $sql_del) or die("クエリの送信に失敗しました。<br />SQL:".$sql_del);
						}
						//かごの中の商品を削除
