...
<ul class="level1">
    ...
    <!--ドコモ決済状況-->
    <!--{if $gmopg_enableDocomo}-->
    <li id="navi-order-gmopg-docomo-status"
        class="<!--{if $tpl_mainno == 'order' && $tpl_subno == 'gmopg_docomo_status'}-->on<!--{/if}-->"
    ><a href="<!--{$smarty.const.ROOT_URLPATH}--><!--{$smarty.const.ADMIN_DIR}-->order/gmopg_docomo_status.php"><span>ドコモケータイ払い状況</span></a></li>
    <!--{/if}-->

    <!--クレジット入力ロック解除-->
    <!--{if $gmopg_enableUseLimit}-->
    <li id="navi-order-gmopg-uselimit-unlock"
        class="<!--{if $tpl_mainno == 'order' && $tpl_subno == 'gmopg_use_limit_unlock'}-->on<!--{/if}-->"
    ><a href="<!--{$smarty.const.ROOT_URLPATH}--><!--{$smarty.const.ADMIN_DIR}-->order/gmopg_use_limit_unlock.php"><span>クレジット入力ロック解除</span></a></li>
    <!--{/if}-->
</ul>
