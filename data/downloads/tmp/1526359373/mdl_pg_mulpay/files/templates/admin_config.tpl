...
<table class="form">
  ...
  <tr>
    <th>認証方式<span class="attention">※</span></th>
    <td>
      <!--{assign var=key value=credit_token}-->
      <span class="attention"><!--{$arrErr[$key]}--></span>
      <input type="radio"
	     name="<!--{$key}-->"
	     value="0" <!--{if $arrForm[$key].value == '0'}-->checked=checked<!--{/if}--> >通常<br />
      <input type="radio"
	     name="<!--{$key}-->"
	     value="1" <!--{if $arrForm[$key].value == '1'}-->checked=checked<!--{/if}--> >トークン方式<br />
      <br /><br />
      <span class="info">※クレジット決済をご利用の場合は、認証方式を選択してください。</span>
    </td>
  </tr>
  <tr>
...
確認のうえ、ご設定いただきますようお願いします。</p>
  ...
  <tr>
    <th>入力回数制限</th>
    <td>
      <!--{assign var=key value="use_limit"}-->
      <span class="attention"><!--{$arrErr[$key]}--></span>
      <input onclick="toggleBox(document.form1.use_limit,'limit_min','limit_count','lock_min');"
	     id="<!--{$key}-->"
	     type="checkbox"
	     name="<!--{$key}-->"
	     style="<!--{$arrErr[$key]|sfGetErrorColor}-->"
	     value="1"
	     maxlength="<!--{$arrForm[$key].length}-->" <!--{if $arrForm[$key].value}--> checked <!--{/if}-->> 使用する場合はチェックを入れてください<br>
      <!--{if $arrForm[$key].value == ''}--><!--{assign var=limit_disable value='disabled="disabled"'}--><!--{/if}-->
    </td>
  </tr>
  <tr>
    <th>入力回数制限<br/>検出時間</th>
    <td>
      <!--{assign var=key value="limit_min"}-->
      <span class="attention"><!--{$arrErr[$key]}--></span>
      <input id="<!--{$key}-->"
	     type="text"
	     name="<!--{$key}-->"
	     class="box6"
	     value="<!--{$arrForm[$key].value}-->"
	     maxlength="<!--{$arrForm[$key].length|escape}-->" <!--{$limit_disable}-->>分<br/><br/>
      <span class="info">※指定した検出時間内に決済エラーが発生した場合、エラー回数を加算します。</span>
    </td>
  </tr>
  <tr>
    <th>入力回数制限<br/>エラー上限回数</th>
    <td>
      <!--{assign var=key value="limit_count"}-->
      <span class="attention"><!--{$arrErr[$key]}--></span>
      <input id="<!--{$key}-->"
	     type="text"
	     name="<!--{$key}-->"
	     class="box6"
	     value="<!--{$arrForm[$key].value}-->"
	     maxlength="<!--{$arrForm[$key].length|escape}-->" <!--{$limit_disable}-->>回<br/><br/>
      <span class="info">※指定したエラー上限回数に達した場合、クレジットロック画面を表示します。</span>
    </td>
  </tr>
  <tr>
    <th>入力回数制限<br/>ロック時間</th>
    <td>
      <!--{assign var=key value="lock_min"}-->
      <span class="attention"><!--{$arrErr[$key]}--></span>
      <input id="<!--{$key}-->"
	     type="text"
	     name="<!--{$key}-->"
	     ...
	     value="<!--{$arrForm[$key].value}-->"
	     maxlength="<!--{$arrForm[$key].length|escape}-->" <!--{$limit_disable}-->>分<br/><br/>
      <span class="info">※エラー上限回数に達した場合のロック時間を設定します。</span>
    </td>
  </tr>
  <tr>
