...
/**
 * GMOクレジット決済状況管理 のページクラス.
 *
 * @package Page
 * @author LOCKON CO.,LTD.
 * @version $Id: LC_Page_Mdl_PG_MULPAY_Admin_Order_Credit_Status.php 3978 2012-03-18 18:26:08Z takashi $
 */
class LC_Page_Mdl_PG_MULPAY_Admin_Order_Credit_Status extends LC_Page_Admin_Ex {
    ...
    function init() {
        ...
        $this->tpl_mainpage = $obj_LC_Mdl_PG_MULPAY_Export->getTplDirPath() . 'admin/order/gmopg_credit_status.tpl';
        $this->tpl_mainno = 'order';
        $this->tpl_subno = 'gmopg_credit_status';
        $this->tpl_subtitle = 'クレジット決済ステータス管理';

        $masterData = new SC_DB_MasterData_Ex();
        $this->arrORDERSTATUS = $masterData->getMasterData("mtb_order_status");
        $this->arrORDERSTATUS_COLOR = $masterData->getMasterData("mtb_order_status_color");

        $this->arrCREDITSTATUS = $obj_LC_Mdl_PG_MULPAY_Export->getCardStatus();
        $this->arrCREDITSTATUS_CHANGE_TABLE = $obj_LC_Mdl_PG_MULPAY_Export->getCardStatusChangeNothingChange();
    ...
    function action() {
        ...
        case 'update':
            ...
            default:
                ...
                */
                ...
                $status = $this->lfStatusMove($status, $objFormParam->getValue('move'));
                break;
            }
        ...
        // 可能な次状態を列挙する
        $this->arrCREDITSTATUS_CHANGE = $this->arrCREDITSTATUS_CHANGE_TABLE[$status];
    ...
    function lfStatusDisp($status,$pageno){
        ...
        $where = "del_flg = " . ($status === "CHECK" ? "1" : "0") . " AND memo04 = ? AND payment_id IN (?, ?)";
        $arrval[] = $status;
        $arrval[] = $obj_LC_Mdl_PG_MULPAY_Export->getGmoCreditPaymentId();
        $arrval[] = $obj_LC_Mdl_PG_MULPAY_Export->getGmoTokenPaymentId();
        $order = "order_id DESC";
    ...
    //ステータス情報の更新
    function lfStatusMove($card_status, $arrOrderId){
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        ...
        if (isset($arrOrderId)) {
            foreach ($arrOrderId as $order_id) {
                if ($order_id != "") {
                    ...
                    // 現状態を問合せる。
                    $responseArray = $obj_LC_Mdl_PG_MULPAY_Export->requestSEARCHTRADE($order_id);
                    ...
                    // 次状態を決定する。
                    $arrUpdate['memo04'] = $card_status;
                    if ($card_status == 'CANCEL') {
                        $cur_state = $responseArray['Status'];
                        $proc_date = $responseArray['ProcessDate'];

                        $card_status = $obj_LC_Mdl_PG_MULPAY_Export->getNextCancelStatus($cur_state, $proc_date);
                        if (!$card_status) {
                            $this->tpl_onload = "window.alert('受注番号$order_idは180日を経過しているのでキャンセルできません。');";
                            return false;
                        }
                        $arrUpdate['memo04'] = $card_status;
                    }
                    ...
                    // 変更リクエストを送信する。
                    $responseArray = $obj_LC_Mdl_PG_MULPAY_Export->requestALTERTRAN($card_status,
                                                                                    $responseArray['Amount'],
                                                                                    $responseArray['Tax'],
                                                                                    $responseArray);
                    if (!is_array($responseArray)) {
                        $this->tpl_onload = "window.alert('受注番号$order_idでエラーが発生しました。\\n$responseArray');";
                        return false;
                    }
        ...
        $message = $this->arrCREDITSTATUS[$card_status] . 'へ変更';
