 ...
 *
 */
class LC_Mdl_PG_MULPAY_Client_Searchcard extends LC_Mdl_PG_MULPAY_Client {
    /**
     * SearchCardリクエストを送信する
     *
     */
    function request($nest_flag = false) {
        if (!$nest_flag) {
            $this->checkMember();
        }
        ...
        $serverUrl = $objPG->getUserSettings('server_url') . 'SearchCard.idPass';
        ...
        $arrSendData = array(
            ...
            'CardSeq'  => '',  // 登録連番.値を入れない場合は登録済みの全データが帰る
        );
    ...
    function checkMember() {
        $objCheckMember = LC_Mdl_PG_MULPAY_Client::factory('Searchmember');
        $objCheckMember->request();
