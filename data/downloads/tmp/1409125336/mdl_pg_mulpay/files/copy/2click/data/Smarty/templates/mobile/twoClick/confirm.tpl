...
<!--{/foreach}-->
<!--{/if}-->
<form action="./deliv.php" method="post">
<input type="hidden" name="<!--{$smarty.const.TRANSACTION_ID_NAME}-->" value="<!--{$transactionid}-->" />
<input type="submit" value="変更">
</form>
...
<!--{else}-->
<font color="#FF0000">配送方法が指定されていません。変更してください。</font><br>
<!--{/if}-->
...
カード番号：<!--{$arrForm.user_confirm_CardNo|h}--><br>
名義人：<!--{$arrForm.user_confirm_HolderName|h}--><br>
有効期限：<!--{$arrForm.user_confirm_Expire|h}--><br>
お支払い回数：<!--{$arrForm.user_confirm_paymethod|h}--><br>
<!--{else}-->
<!--{$arrForm.user_confirm|h}--><br>
<!--{/if}-->
<!--{else}-->
<font color="#FF0000">お支払い方法が指定されていません。変更してください。</font><br>
<!--{/if}-->
...
<!--{if $arrForm.deliv_id && $arrForm.payment_id}-->
...
<center><input type="submit" value="ご注文完了ページへ"></center>
</form>
<form action="<!--{$smarty.const.MOBILE_CART_URLPATH}-->" method="post">
<input type="hidden" name="<!--{$smarty.const.TRANSACTION_ID_NAME}-->" value="<!--{$transactionid}-->" />
