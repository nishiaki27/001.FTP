 ...
 *
 */
class LC_Mdl_PG_MULPAY_Client_Exec_Webmoney extends LC_Mdl_PG_MULPAY_Client_Exec {
    /**
     * ExecTran名を取得する
     *
    ...
    function getExecTranName() {
        return 'ExecTranWebmoney.idPass';
    }
    ...
    function getSendRequestParam($arrEntryRet, $objPage) {
        ...
        $itemName = LC_Mdl_PG_MULPAY_Utils::subString($orderNo . ' ' . $tempProductName, WEBMONEY_ITEM_NAME);
        //$objPG->printLog("WEBMONEY_ITEM_NAME: $itemName");

        // 加盟店自由項目
        $clientField1 = $objPG->getUserSettings('webmoney_ClientField1');
        $clientField2 = $objPG->getUserSettings('webmoney_ClientField2');
        ...
        $arrSendData = array(
            ...
            //'ShopMailAddress'
            'RedirectURL'	  => $this->getModuleRetUrl(array('paypal_order_id' => $objPage->arrData['order_id'])),
        ...
        // GMOPGから購入者へのメール
        if (MDL_PG_MULPAY_CONF_PGMAIL_WEBMONEY) {
            //$objPG->printLog('MDL_PG_MULPAY_CONF_PGMAIL_WEBMONEY');
            $arrSendData['MailAddress'] = $objPage->arrData['order_email'];
        }
        // 支払い期限
        if (strlen($objPG->getUserSettings('webmoney_PaymentTermDay')) !== 0) {
            $arrSendData['PaymentTermDay'] = $objPG->getUserSettings('webmoney_PaymentTermDay');
        }
