 ...
 *
 */
class LC_Mdl_PG_MULPAY_Client_Exec_PayPal extends LC_Mdl_PG_MULPAY_Client_Exec {
    /**
     * ExecTran名を取得する
     *
    ...
    function getSendRequestParam($arrEntryRet, $objPage) {
        ...
        $itemName = LC_Mdl_PG_MULPAY_Utils::subString($orderNo . ' ' . $tempProductName, PAYPAL_ITEM_NAME);
        //$objPG->printLog("PAYPAL_ITEM_NAME: $itemName");

        // 加盟店自由項目
        $clientField1 = $objPG->getUserSettings('paypal_ClientField1');
        $clientField2 = $objPG->getUserSettings('paypal_ClientField2');
        ...
        $arrSendData = array(
            ...
            'TelNo'           => $telNo,
            'ItemName'		  => $itemName,
            'RedirectURL'	  => $this->getModuleRetUrl(),
        ...
        // GMOPGから購入者へのメール
        if (MDL_PG_MULPAY_CONF_PGMAIL_PAYPAL) {
            //$objPG->printLog('MDL_PG_MULPAY_CONF_PGMAIL_PAYPAL');
