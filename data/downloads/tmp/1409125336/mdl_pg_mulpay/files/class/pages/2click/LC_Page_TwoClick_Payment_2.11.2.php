 ...
 * @version $Id: LC_Page_TwoClick_Payment_2.11.2.php 3212 2011-10-19 11:44:08Z takashi $
 */
class LC_Page_TwoClick_Payment extends LC_Page_Ex {
    ...
    function init() {
        ...
        $this->tpl_mainpage = 'twoClick/payment.tpl';
    }
        ...
        case 'confirm':
            ...
            if (SC_Utils_Ex::isBlank($this->arrErr)) {
                ...
                if (LC_Mdl_PG_MULPAY_Export::isGmoPaymentId($payment_id)) {
                    // 決済モジュールページ（支払い方法詳細）へ移動
                    $_SESSION['payment_id'] = $payment_id;
                    SC_Response_Ex::sendRedirect('./load_payment_module.php');
                } else {
                    // 確認ページへ移動
                    // XXX define('SHOPPING_CONFIRM_URLPATH', ROOT_URLPATH . "shopping/confirm.php");
                    SC_Response_Ex::sendRedirect(ROOT_URLPATH . 'twoClick/confirm.php');
                }
                exit;
            } else {
                // 受注一時テーブルからの情報を格納
    ...
    function lfRegistData($uniqid, $arrForm, &$objPurchase, $arrPayment) {
        ...
        // memo06は決済実行時に必要に応じて設定されるので、ここでクリアしておく。
        $arrForm['memo06'] = '';
    ...
    function getSelectedDeliv(&$objPurchase, &$objCartSess, $deliv_id) {
        ...
        $arrResults['arrPayment'] = $objPurchase->getPaymentsByPrice($total,
                                                                     $deliv_id);
        // GMOモジュール決済以外のモジュール支払いを除外する。
        $gmoPayments = LC_Mdl_PG_MULPAY_Export::getAllGmoPayments();
        $idhash = array();
        foreach ($gmoPayments as $val) {
            $idhash[$val['payment_id']] = true;
        }
        $altPayments = array();
        foreach ($arrResults['arrPayment'] as $val) {
            if ($idhash[$val['payment_id']]) {
                $altPayments[] = $val;
            }
        }
        $arrResults['arrPayment'] = $altPayments;
    ...
    function getMobileMainpage($is_single_deliv = true, $mode) {
        ...
        default:
            if ($is_single_deliv) {
                ...
                return 'twoClick/select_deliv.tpl';
            }
        }
    }
}
