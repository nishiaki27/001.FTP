 ...
 *
 */
class LC_Mdl_PG_MULPAY_Client_Savemember extends LC_Mdl_PG_MULPAY_Client {
    /**
    ...
    function request() {
        ...
        $serverUrl = $objPG->getUserSettings('server_url') . 'SaveMember.idPass';
    ...
    function parse($ret) {
        ...
        if (isset($arrRet[0]['ErrCode'])) {
            // エラーが『「登録済み」だけ』の場合はエラーとして扱わない
            if ($arrRet[0]['ErrCode'] == 'E01'
            && $arrRet[0]['ErrInfo'] == 'E01390010'
            && !isset($arrRet[1])) {

                $objPG =& LC_Mdl_PG_MULPAY::getInstance();
                $objPG->printLog('-> already register customer');
                return;
            }
