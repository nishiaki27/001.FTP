...
<div id="order" class="contents-main">
    <h2>抽出条件</h2>
        <div class="btn">
        <!--{foreach key=key item=item from=$arrCREDITSTATUS}-->
            <a
                class="btn-normal"
    ...
    <!--{if $tpl_linemax > 0}-->
        <div class="btn">
            <select name="change_status">
                ...
                <!--{foreach key=key item=item from=$arrCREDITSTATUS_CHANGE}-->
...
<div>
<!--{if $SelectedStatus eq "AUTH"}-->
仮売上<br/>
&nbsp;&nbsp;カードの与信枠を確保し承認番号を得ること。<br/>
&nbsp;&nbsp;※仮売上のデータ保持期間は90日です。<br/>
&nbsp;&nbsp;実売上処理を行わないとカード会社への売上データが作成されません。<br/>
<!--{elseif $SelectedStatus eq "SALES"}-->
<br/>実売上<br/>
&nbsp;&nbsp;仮売上処理が行われたデータに対し、カード会社への売上データの作成依頼をすること。 <br/>
&nbsp;&nbsp;※仮売上から90日間を経過したものは実売上処理できません。 <br/>
<!--{elseif $SelectedStatus eq "CAPTURE"}-->
<br/>即時売上<br/>
&nbsp;&nbsp;カードの与信枠を確保し承認番号を得て、カード会社への売上データの作成依頼をすること。<br/>
&nbsp;&nbsp;（仮売上+実売上の処理になります。）<br/>
<!--{elseif $SelectedStatus eq "VOID"}-->
<br/>取消<br/>
&nbsp;&nbsp;仮売上、実売上、即時売上の処理を行った当日におこなうキャンセル<br/>
&nbsp;&nbsp;※実売上の場合、該当の仮売上と同日に処理をした場合<br/>
<!--{elseif $SelectedStatus eq "RETURN"}-->
<br/>返品<br/>
&nbsp;&nbsp;仮売上の処理を行った翌日以降におこなうキャンセル。<br/>
&nbsp;&nbsp;実売上の場合は、該当の仮売上の翌日以降〜当月内におこなうキャンセル。<br/>
&nbsp;&nbsp;売上の処理を行った翌日以降〜当月内におこなうキャンセル。<br/>
<!--{elseif $SelectedStatus eq "RETURNX"}-->
<br/>月跨返品<br/>
&nbsp;&nbsp;実売上、即時売上の処理を行った翌月以降に行うキャンセル<br/>
&nbsp;&nbsp;※仮売上に対する月跨返品はできません。<br/>
<!--{elseif $SelectedStatus eq "CHECK"}-->
<br/>有効性チェック<br/>
&nbsp;&nbsp;カードの有効性（使用できるか否か）のチェックのみを行うこと。<br/>
&nbsp;&nbsp;（利用金額を設定してもカード会社へは送信されません。）<br/>
&nbsp;&nbsp;※カード会社によっては承認番号を返却しない場合もございます。<br/>
&nbsp;&nbsp;この場合はエラーコードを返却しないことが許可（OK）になります。<br/>
<!--{/if}-->
</div>
...
*}-->
            ...
            <tr style="background:<!--{$arrORDERSTATUS_COLOR[$status]}-->;">
                ...
                <td><a href="#" onclick="fnOpenWindow('./edit.php?order_id=<!--{$arrStatus[cnt].order_id}-->','order_disp','800','900'); return false;" ><!--{$arrStatus[cnt].order_id}--></a></td>
                <td><!--{$arrStatus[cnt].create_date|sfDispDBDate:false}--></td>
                <td><!--{$arrStatus[cnt].order_name01|h}--><!--{$arrStatus[cnt].order_name02|h}--></td>
                <!--{assign var=credit_status value="`$arrStatus[cnt].memo04`"}-->
                <td><!--{$arrCREDITSTATUS[$credit_status]|h}--></td>
