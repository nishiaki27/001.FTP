 ...
 *}-->
<!--{*
  $Id: cart_index_2.11.2.tpl 2929 2011-08-15 06:32:21Z takashi $
 *}-->
    ...
    <div class="form_area">
        <form name="form<!--{$key}-->" id="form<!--{$key}-->" method="post" action="?">
		    ...
		    <!--{if $isEnable2click}-->
                    <li>
                        <!--{if strlen($tpl_error) == 0}-->
                        <input type="image" onmouseover="chgImgImageSubmit('<!--{$TPL_URLPATH}-->img/button/btn_2click_on.jpg',this)" onmouseout="chgImgImageSubmit('<!--{$TPL_URLPATH}-->img/button/btn_2click.jpg',this)" src="<!--{$TPL_URLPATH}-->img/button/btn_2click.jpg" alt="2クリック購入手続きへ" name="mdl_pg_mulpay_2click_confirm" onclick="document.form<!--{$key}-->.mode.value = 'mdl_pg_mulpay_2click';document.form<!--{$key}-->.submit();" />
                        <!--{/if}-->
                    </li>
		    <!--{/if}-->
                </ul>
