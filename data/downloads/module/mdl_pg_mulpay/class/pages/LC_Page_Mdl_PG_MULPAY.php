...
require_once(MDL_PG_MULPAY_CLASS_REALDIR . 'utils/LC_Mdl_PG_MULPAY_AccountLock.php');
...
//    	}
     ...
     * クレジット入力のロックを行いエラー画面にリダイレクトする
     */
    function checkErrorLimit() {
        $objPG =& LC_Mdl_PG_MULPAY::getInstance();

        // 入力回数制限機能を利用しない場合はここまで
        $use_limit = $objPG->getUserSettings('use_limit');
        if (!$use_limit || $use_limit != "1") {
            return;
        }
        ...
        $objPG->printLog(print_r($objAL->getAccountInfo(), true));
...
EOS;
    ...
    function unsetUseLimitOrderTemp() {
        $objPurchase = new SC_Helper_Purchase_Ex();
        $objQuery =& SC_Query_Ex::getSingletonInstance();

        $arrVal = $objPurchase->getOrderTempByOrderId($_SESSION['order_id']);
        $ses = unserialize($arrVal['session']);
        unset($ses['use_limit']);
        $sqlVal['session'] = serialize($ses);
        $objQuery->update('dtb_order_temp', $sqlVal, 'order_temp_id = ?', array($arrVal['order_temp_id']));
    }
}
?>
