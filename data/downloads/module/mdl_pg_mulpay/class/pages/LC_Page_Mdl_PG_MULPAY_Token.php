...
/**
 * トークン決済情報入力画面 のページクラス.
 *
 * @package Page
 */
class LC_Page_Mdl_PG_MULPAY_Token extends LC_Page_Mdl_PG_MULPAY {
    /**
     * Page を初期化する.
     *
     * @return void
    ...
    function setTemplateVars() {
        ...
        $this->js_urlpath = $this->getJsUrlPath($objPG);
        $this->tshop = $objPG->getUserSettings('shop_id');
    }
    ...
    function registerMode() {
        ...
        $objEntry = LC_Mdl_PG_MULPAY_Client::factory('Entry_Token');
        ...
        if ($objForm->getValue('register_card')) {
            $useCard = true;
            $arrForm['CardSeq'] = $savedCard[0]['CardSeq'];
        }

        $objExec = LC_Mdl_PG_MULPAY_Client::factory('Exec_Token');
    ...
    function initParamAdd(&$objForm) {
        $objForm->addParam('カード番号', 'card_no', CREDIT_NO_MAX_LEN, 'n', array('MAX_LENGTH_CHECK', 'NUM_CHECK'));
        $objForm->addParam("カード期限年", "card_year", 2, "n", array("NUM_COUNT_CHECK", "NUM_CHECK"));
        $objForm->addParam("カード期限月", "card_month", 2, "n", array("NUM_COUNT_CHECK", "NUM_CHECK"));
        $objForm->addParam("名", "card_name01", 25, "KVa", array("MAX_LENGTH_CHECK", "ALPHA_CHECK"));
        $objForm->addParam("姓", "card_name02", 24, "KVa", array("MAX_LENGTH_CHECK", "ALPHA_CHECK"));

        $objForm->addParam('トークン', 'token', 0, '', array('EXIST_CHECK'));
    ...
    function getJsUrlPath(&$objPG) {
        $server_url = $objPG->getUserSettings('server_url');
