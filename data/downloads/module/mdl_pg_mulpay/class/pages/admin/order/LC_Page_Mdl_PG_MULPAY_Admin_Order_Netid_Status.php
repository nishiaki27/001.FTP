 ...
 * @version $Id: LC_Page_Mdl_PG_MULPAY_Admin_Order_Netid_Status.php 3978 2012-03-18 18:26:08Z takashi $
 */
class LC_Page_Mdl_PG_MULPAY_Admin_Order_Netid_Status extends LC_Page_Admin_Ex {
    ...
    function init() {
        ...
        $this->tpl_mainpage = $obj_LC_Mdl_PG_MULPAY_Export->getTplDirPath() . 'admin/order/gmopg_netid_status.tpl';
        $this->tpl_mainno = 'order';
        $this->tpl_subno = 'gmopg_netid_status';
        $this->tpl_subtitle = 'iD決済ステータス管理';

        $masterData = new SC_DB_MasterData_Ex();
        $this->arrORDERSTATUS = $masterData->getMasterData("mtb_order_status");
        $this->arrORDERSTATUS_COLOR = $masterData->getMasterData("mtb_order_status_color");

        $this->arrNETIDSTATUS = $obj_LC_Mdl_PG_MULPAY_Export->getNetidStatusArray();
        $this->arrNETIDSTATUS_CHANGE_TABLE = $obj_LC_Mdl_PG_MULPAY_Export->getNetidStatusChangeArray();

        $objPG =& LC_Mdl_PG_MULPAY::getInstance();
        $this->gmopg_enableCardStatusChange = $objPG->isEnableCardStatusChangeFunction();
        $this->gmopg_enableNetid = $objPG->isEnableNetid();
    }
    ...
    function action() {
        ...
        // 可能な次状態を列挙する
        $this->arrNETIDSTATUS_CHANGE = $this->arrNETIDSTATUS_CHANGE_TABLE[$status];
    ...
    function lfStatusDisp($status,$pageno){
        ...
        $arrval[] = $status;
        $arrval[] = $obj_LC_Mdl_PG_MULPAY_Export->getGmoNetidPaymentId();
        $order = "order_id DESC";
    ...
    //ステータス情報の更新
    function lfStatusMove($netid_status, $arrOrderId){
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $arrUpdate = array('update_date' => 'NOW()');
        if ($netid_status != 'CHANGE') {
            $arrUpdate['memo04'] = $netid_status;
        }
        
        //$objPG =& LC_Mdl_PG_MULPAY::getInstance();
        //$objPG->printLog("netid: $netid_status");
        ...
        if (isset($arrOrderId)) {
            foreach ($arrOrderId as $order_id) {
                if ($order_id != "") {
                    ...
                    $responseArray = $obj_LC_Mdl_PG_MULPAY_Export->requestChangeNetidStatus($order_id, $netid_status);
        ...
        $message = $this->arrNETIDSTATUS[$netid_status] . 'へ変更';
