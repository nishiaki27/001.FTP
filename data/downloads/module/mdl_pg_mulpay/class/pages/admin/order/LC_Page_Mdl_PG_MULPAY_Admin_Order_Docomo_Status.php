...
/**
 * ドコモケータイ払い状況管理 のページクラス.
 *
 * @package Page
 * @author Spirit of Co.,Ltd.
 * @version $Id$
 */
class LC_Page_Mdl_PG_MULPAY_Admin_Order_Docomo_Status extends LC_Page_Admin_Ex {
    ...
    function init() {
        ...
        $this->tpl_mainpage = $obj_LC_Mdl_PG_MULPAY_Export->getTplDirPath() . 'admin/order/gmopg_docomo_status.tpl';
        $this->tpl_mainno = 'order';
        $this->tpl_subno = 'gmopg_docomo_status';
        $this->tpl_subtitle = 'ドコモケータイ払いステータス管理';

        $masterData = new SC_DB_MasterData_Ex();
        $this->arrORDERSTATUS = $masterData->getMasterData("mtb_order_status");
        $this->arrORDERSTATUS_COLOR = $masterData->getMasterData("mtb_order_status_color");

        $this->arrDocomoSTATUS = $obj_LC_Mdl_PG_MULPAY_Export->getDocomoStatusArray();
        $this->arrDocomoSTATUS_CHANGE_TABLE = $obj_LC_Mdl_PG_MULPAY_Export->getDocomoStatusChangeArray();
        $this->arrDocomoPAYMETHOD = $obj_LC_Mdl_PG_MULPAY_Export->getDocomoPayMethodArray();

        $objPG =& LC_Mdl_PG_MULPAY::getInstance();
        $this->gmopg_enableCardStatusChange = $objPG->isEnableCardStatusChangeFunction();
        $this->gmopg_enableDocomo = $objPG->isEnableDocomo();
    }
    ...
    function action() {
        ...
        case 'update':
            ...
            default:
                ...
                */
                $status = $objFormParam->getValue('change_status'); // 次状態
                if (! $this->lfStatusMove($status, $objFormParam->getValue('move'))) {
                    // 失敗時は次状態の画面に遷移しない。
        ...
        // 可能な次状態を列挙する
        $this->arrDocomoSTATUS_CHANGE = $this->arrDocomoSTATUS_CHANGE_TABLE[$status];
    ...
    function lfStatusDisp($status,$pageno){
        ...
        $arrval[] = $status;
        $arrval[] = $obj_LC_Mdl_PG_MULPAY_Export->getGmoDocomoPaymentId();
        $order = "order_id DESC";
    ...
    //ステータス情報の更新
    function lfStatusMove($docomo_status, $arrOrderId){
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $arrUpdate = array('update_date' => 'NOW()');
        $arrUpdate['memo04'] = $docomo_status;

        $objPG =& LC_Mdl_PG_MULPAY::getInstance();
        $objPG->printLog("docomo: $docomo_status");
        ...
        if (isset($arrOrderId)) {
            foreach ($arrOrderId as $order_id) {
                if ($order_id != "") {
                    $responseArray = $obj_LC_Mdl_PG_MULPAY_Export->requestChangeDocomoStatus($order_id, $docomo_status);
                    ...
                    $arrUpdate['memo04'] = $responseArray['Status'];
        ...
        $message = $this->arrDocomoSTATUS[$docomo_status] . 'へ変更';
