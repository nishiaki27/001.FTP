...
class LC_Page_Mdl_PG_MULPAY_Conveni extends LC_Page_Mdl_PG_MULPAY {
    ...
    function getSerializeMessage($arrExecRet) {
        ...
        if ($this->arrForm['conveni'] === CONVENI_LOSON ||
            $this->arrForm['conveni'] === CONVENI_MINISTOP) {
            $message['receipt_no'] = $this->lfSetConvMSG("お客様番号", $arrExecRet['ReceiptNo']);
        } else if ($this->arrForm['conveni'] === CONVENI_CIRCLEK ||
                   $this->arrForm['conveni'] === CONVENI_SUNKUS) {
            $message['receipt_no'] = $this->lfSetConvMSG("受付番号", substr($arrExecRet['ReceiptNo'], 0, 4) . '-' . substr($arrExecRet['ReceiptNo'], 4));
        } else if ($this->arrForm['conveni'] === CONVENI_DAILYYAMAZAKI ||
                   $this->arrForm['conveni'] === CONVENI_SEVENELEVEN) {
            $message['receipt_no'] = $this->lfSetConvMSG("オンライン決済番号", substr($arrExecRet['ReceiptNo'], 0, 4) . '-' . substr($arrExecRet['ReceiptNo'], 4));
        }

        if ($this->arrForm['conveni'] === CONVENI_FAMILYMART) {
            $message['conf_no'] = $this->lfSetConvMSG("企業コード", $arrExecRet['ConfNo']);
        } else {
            $message['conf_no'] = $this->lfSetConvMSG("確認番号", $arrExecRet['ConfNo']);
        }

        if ($this->arrForm['conveni'] === CONVENI_FAMILYMART) {
            $message['receipt_no'] = $this->lfSetConvMSG("注文番号", $arrExecRet['ReceiptNo']);
        }

        if ($this->arrForm['conveni'] === CONVENI_SEVENELEVEN) {
            $message['receipt_url'] = $this->lfSetConvMSG("払込票URL", $arrExecRet['ReceiptUrl']);
        }
