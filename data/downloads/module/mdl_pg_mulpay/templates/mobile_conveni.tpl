...
<!--{if $arrErr[$key]}-->
<font color="red"><!--{$arrErr[$key]|nl2br}--></font>
<br>
<!--{/if}-->
<!--{section name=cnt loop=$arrUseConveni}-->
<!--{assign var=val value=$arrUseConveni[cnt]}-->
<input type="radio" name="<!--{$key}-->" value="<!--{$arrUseConveni[cnt]}-->" style="<!--{$arrErr[$key]|sfGetErrorColor}-->" <!--{$arrUseConveni[cnt]|sfGetChecked:$arrForm[$key].value}--> />
<!--{$arrCONVENI[$val]|escape}-->
<br>
<!--{/section}-->
<br>
<!--{if $is2clickFlow}-->
