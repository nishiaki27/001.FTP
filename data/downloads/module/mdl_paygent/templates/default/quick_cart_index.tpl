    ...
    <div class="form_area">
        <form name="form<!--{$key}-->" id="form<!--{$key}-->" method="post" action="?">
            ...
            <table summary="商品情報">
                ...
                <!--{/foreach}-->
                <tr>
                    <th colspan="5" class="Right bg_cor">小計</th>
                    <td class="bg_yel"><!--{$tpl_total_inctax[$key]|number_format}-->円</td>
                </tr>
                <tr>
                    <th colspan="5" class="Left">合計</th>
                    <td class="bg_yel txt_R"><span class="price"><!--{$arrData[$key].total-$arrData[$key].deliv_fee|number_format}-->円</span></td>
                </tr>
                <!--{if $smarty.const.USE_POINT !== false}-->
                ...
                <!--{if strlen($tpl_error) == 0}-->
                    <p class="alignC">上記内容でよろしければ「購入手続きへ」ボタンをクリックしてください。
			<!--{if $tpl_quick[$key] == "1"}-->
            ...
            <div class="btn_area">
                <ul>
                    <li>
                        <!--{if $tpl_prev_url != ""}-->
                            <a href="<!--{$tpl_prev_url|h}-->" onmouseover="chgImg('<!--{$TPL_URLPATH}-->img/button/btn_inq_back.png','back<!--{$key}-->');" onmouseout="chgImg('<!--{$TPL_URLPATH}-->img/button/btn_inq_back.png','back<!--{$key}-->');">
                                <img src="<!--{$TPL_URLPATH}-->img/button/btn_inq_back.png" alt="戻る" name="back<!--{$key}-->" /></a>
                        <!--{/if}-->
                    </li>
                    <li>
                        <!--{if strlen($tpl_error) == 0}-->
                            <input type="hidden" name="cartKey" value="<!--{$key}-->" />
                            <input type="image" onmouseover="chgImgImageSubmit('<!--{$TPL_URLPATH}-->img/button/btn_buystep_on.png',this)" onmouseout="chgImgImageSubmit('<!--{$TPL_URLPATH}-->img/button/btn_buystep_on.png',this)" src="<!--{$TPL_URLPATH}-->img/button/btn_buystep_on.png" alt="購入手続きへ" name="confirm" />
                        <!--{/if}-->
                    </li>
		    <!--{if $tpl_quick[$key] == "1"}-->
                    <li>
                        <!--{if strlen($tpl_error) == 0}-->
                            <input type="hidden" name="cartKey" value="<!--{$key}-->" />
			    <a href="?" onclick="fnFormModeSubmit('form<!--{$key}-->','quick','',''); return false"><img onmouseover="chgImgImageSubmit('<!--{$TPL_URLPATH}-->img/button/btn_quickbuy_on.jpg',this)" onmouseout="chgImgImageSubmit('<!--{$TPL_URLPATH}-->img/button/btn_quickbuy_off.jpg',this)" src="<!--{$TPL_URLPATH}-->img/button/btn_quickbuy_off.jpg" /></a>
                        <!--{/if}-->
                    </li>
		    <!--{/if}-->
                </ul>
