...
class LC_Page_Mdl_PG_MULPAY extends LC_Page_Ex {
    ...
    function action() {
        ...
        $this->checkDelFlg($this->objSiteSess);
...
//    	}
    ...
    function checkDelFlg(&$objSiteSess) {
        // 2クリックモードではチェックする必要がない。
        if ($this->is2clickFlow) {
            return;
        }

        // この受注IDのdel_flgが0でない場合は、
        // 不正な画面遷移なので、購入フローを強制終了する。
        $objPurchase = new SC_Helper_Purchase_Ex();
        $arrOrder = $objPurchase->getOrder($_SESSION['order_id']);
        //LC_Mdl_PG_MULPAY::printLog('checkDelFlg del_flg: '.print_r($arrOrder['del_flg'],true));

        if ($arrOrder['del_flg'] != 0) {
            LC_Mdl_PG_MULPAY::printLog('checkDelFlg detect error condition. del_flg='.print_r($arrOrder['del_flg'],true));
