 ...
 * @version $Id: LC_Page_Mdl_PG_MULPAY_Admin_Order_Au_Status.php 4139 2012-04-24 05:21:13Z takashi $
 */
class LC_Page_Mdl_PG_MULPAY_Admin_Order_Au_Status extends LC_Page_Admin_Ex {
    ...
    function init() {
        ...
        $this->tpl_mainpage = $obj_LC_Mdl_PG_MULPAY_Export->getTplDirPath() . 'admin/order/gmopg_au_status.tpl';
        $this->tpl_mainno = 'order';
        $this->tpl_subno = 'gmopg_au_status';
        $this->tpl_subtitle = 'auかんたん決済ステータス管理';

        $masterData = new SC_DB_MasterData_Ex();
        $this->arrORDERSTATUS = $masterData->getMasterData("mtb_order_status");
        $this->arrORDERSTATUS_COLOR = $masterData->getMasterData("mtb_order_status_color");

        $this->arrAUSTATUS = $obj_LC_Mdl_PG_MULPAY_Export->getAuStatusArray();
        $this->arrAUSTATUS_CHANGE_TABLE = $obj_LC_Mdl_PG_MULPAY_Export->getAuStatusChangeArray();
        $this->arrAUPAYMETHOD = $obj_LC_Mdl_PG_MULPAY_Export->getAuPayMethodArray();

        $objPG =& LC_Mdl_PG_MULPAY::getInstance();
        $this->gmopg_enableCardStatusChange = $objPG->isEnableCardStatusChangeFunction();
        $this->gmopg_enableAu = $objPG->isEnableAu();
    }
    ...
    function action() {
        ...
        // 可能な次状態を列挙する
        $this->arrAUSTATUS_CHANGE = $this->arrAUSTATUS_CHANGE_TABLE[$status];
    ...
    function lfStatusDisp($status,$pageno){
        ...
        $arrval[] = $status;
        $arrval[] = $obj_LC_Mdl_PG_MULPAY_Export->getGmoAuPaymentId();
        $order = "order_id DESC";
    ...
    //ステータス情報の更新
    function lfStatusMove($au_status, $arrOrderId){
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $arrUpdate = array('update_date' => 'NOW()');
        $arrUpdate['memo04'] = $au_status;

        $objPG =& LC_Mdl_PG_MULPAY::getInstance();
        $objPG->printLog("au: $au_status");
        ...
        if (isset($arrOrderId)) {
            foreach ($arrOrderId as $order_id) {
                if ($order_id != "") {
                    $responseArray = $obj_LC_Mdl_PG_MULPAY_Export->requestChangeAuStatus($order_id, $au_status);
        ...
        $message = $this->arrAUSTATUS[$au_status] . 'へ変更';
