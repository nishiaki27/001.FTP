...
class LC_Page_User extends LC_Page_Ex {
    ...
    function action() {
        ...
        $htmlData = file_get_contents($_SERVER['DOCUMENT_ROOT'] . '/include/industrial_aircon_discount/'. $type .'.txt');
        if ($htmlData) {
...
*/
    ...
    function getResults($arrParams = array()) {
        ...
        $from .= "JOIN dtb_products_class AS T3 ";
        $from .= "ON alldtl.product_id = T3.product_id ";

        $where  = "alldtl.del_flg = 0 ";
        $where .= "AND alldtl.status = 1 ";
        $where .= "AND alldtl.comment1 is NULL ";
        $where .= "AND T3.price02 >= 378000 ";
        $arrValues = array();
