...
$table_list[] = array(
	...
	'select_table_sql' => "dtb_products_class",
...
foreach($table_list as $update_table){
	...
	try {
		//CSVファイル作成（ヘッダー付き）
		$copy_stmt = $pdo->prepare("COPY ".$select_tablesql." TO '".$csv_filename."' WITH CSV DELIMITER ',' HEADER");
		$copy_stmt -> execute();
		...
		try {
			...
			$del_stmt = $pdo2->prepare('DELETE FROM `'.$server."_temp".'`');
			$del_stmt -> execute();
